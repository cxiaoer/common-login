...
class Jd(object):
    ...
    def login(self):
        soup = BeautifulSoup(
            session.get('https://passport.jd.com/new/login.aspx', verify=False, headers=header).content)
        ...
        res = session.post('https://passport.jd.com/uc/loginService', params=params, data=data, headers=header,
                           verify=False)
        print res.url
        print res.content
